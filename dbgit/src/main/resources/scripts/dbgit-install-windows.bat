...
if defined ValueValue (
    ...
    Powershell.exe -executionpolicy remotesigned -File  %~dp0\git-download.ps1 %~dp0git-install.exe
    echo Installing git...
...
if NOT %INSTALL_PATH% == %~dp0 (
    ...
    Powershell.exe -executionpolicy remotesigned -File  %~dp0\path_update.ps1 %INSTALL_PATH%\dbgit
) else (
    echo Adding to PATH variable...
    Powershell.exe -executionpolicy remotesigned -File  %~dp0\path_update.ps1 %INSTALL_PATH%bin
)
