            ...
            return new CharsWithMaskedCredentials(new CharsOf<Subj>(() -> {
                try {
                    ...
                    try {
                        ...
                        return new ReportOfTestFormat(label, test.description(), subjDetails, testDetails);

                    } catch (SafeScalarOf.SafeScalarError subjectThrowable) {
                        return new ReportOfTestFormat(
                            new LabelOfTestRunBrokenSubject()
                            , test.description()
                            , ExceptionUtils.readStackTrace(subjectThrowable.getCause())
                        );
                ...
                } catch (CharsOfConsoleWhenRunning.CharsOfConsoleWhenRunningException subjectThrowable) {
                    return new ReportOfTestFormat(
                        new LabelOfTestRunBrokenSubject()
                        , test.description()
                        , ExceptionUtils.readStackTrace(subjectThrowable)
                    );
                }
            }));
        });
