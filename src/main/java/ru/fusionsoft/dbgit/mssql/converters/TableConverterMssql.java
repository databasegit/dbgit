...
public class TableConverterMssql implements IDBConvertAdapter {
	...
	private String indexFromOracle(DBIndex index) {
		ConsoleWriter.println("Converting table index " + index.getName() + " from oracle to MsSql...");

		return "";
	}

	private String constraintFromOracle(DBConstraint constraint) {
		ConsoleWriter.println("Converting table constraint " + constraint.getName() + " from oracle to MSSql...");
	...
	private String typeFromOracle(DBTableField field) {
		ConsoleWriter.println("Converting table field " + field.getName() + " from oracle to MsSql...");

		String result = "";

		//TODO MSSQL type names
